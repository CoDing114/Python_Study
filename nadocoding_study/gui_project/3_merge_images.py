...
# root.geometry("340x480+1100+100")    # 가로 * 세로 크기 + x좌표 +y좌표
...
def add_file():
    files = filedialog.askopenfilenames(
        title="이미지 파일을 선택 하세요", filetypes=(("PNG", "*.png"), ("모든 파일", "*.*")), initialdir=r"d:\Sung\Pictures")
    # 경로 설정때 앞에r을 붙이면 뒤에 오는 \그래도 쓰면된다.
...
def merge_image():
    ...
    # widths = [x.size[0] for x in images]    # 각 이미지 의 너비 구하기
    # heights = [x.size[1] for x in images]
    widths, heights = zip(*(x.size for x in images))

    max_width, total_height = max(widths), sum(heights)  # 젤큰 너비와 , 총 높이를 구한다

    # 스케치북 준비
    result_img = Image.new(
        "RGB", (max_width, total_height), (255, 255, 255))  # 배경 흰색
    y_offset = 0    # 이미지 y 좌표(위치정보)
    # for img in images:
    #     result_img.paste(img, (0, y_offset))
    #     y_offset += img.size[1]  # heigth 값 만큼 더해줌
...
# 진행 상황 Progress Bar
frame_progress = LabelFrame(root, text="진행현황")
...
root.resizable(False, False)    # 스크린 사이즈 변경 불가
